...
- complete one-Mission product story: [v1 unified flagship](v1-flagship-local-2026-08-26.json);
...
- installed-product and source-bundle reproduction: [Iteration 10 package
  smoke](https://github.com/Oxygen56/missionbraid/blob/main/evidence/iteration-10-package-smoke-local-2026-08-26.json).

The v1 flagship record is the first retained run that connects the major
Workbench capabilities under one Mission identity. Against a clean worktree at
revision `5aac506`, one same-host controlled fixture uses real installed
Qoder/Qwen3.8-Max and Claude Code/deepseek-v4-pro processes to establish:

- a declared failed Qoder source Attempt, a recorded manual target override
  applied by the deterministic Planner to bind Claude, Handoff acknowledgement
  before the target tool boundary, a native Claude pre-tool Write modification,
  and a Receipt rejected only on stale Context;
- exactly one POST to a queryable external Effect target, followed by controller
  termination and lookup-based reconciliation without a second POST;
- a Git-backed Composite Checkpoint, Context-only Execution Fork, verified
  child Receipt, confirmed stale-Context mechanism, evidence ablation back to
  inferred, and an honest unknown candidate;
- a revised-Branch selection recorded with declared human authority by the
  controlled fixture; three verified real trials after Planner Profile-Rebound
  from the source Claude Profile to a distinct declared higher-reasoning Claude
  Profile with the same native tool-control capability; and a standalone checker
  that exits 0 for the retained result and 1 when a required Effect remains
  unresolved;
...
The provider termination and exit-17 tool probe are deliberately induced
observation boundaries. This is one local same-host controlled-fixture result,
and the selection authority is a fixture-declared field rather than proof of
live human interaction or identity verification. It is not a natural-failure
reliability rate, independent third-party or cross-host reproduction,
provider-internal Context capture, production adoption, npm publication, or
...
lockfile through frozen install, typecheck, build, and 374 tests without
...
| ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [v1 unified flagship](v1-flagship-local-2026-08-26.json)                                                                                   | One clean-revision, same-host local real-multi-Runtime controlled-fixture run  | One Mission at `5aac506`: Qoder→Claude Handoff/tool gate/rejection; one-POST Effect recovery; Checkpoint/Fork/diagnosis; three real Claude Profile-Rebound trials; CI exits 0/1; revised Plan/reuse/consolidation/Receipt/restart | Natural-failure or general reliability, provider-internal Context, independent third-party or cross-host reproduction, production adoption, npm publication  |
| [Iteration 10 package smoke](https://github.com/Oxygen56/missionbraid/blob/main/evidence/iteration-10-package-smoke-local-2026-08-26.json) | Internal clean install of one recorded local package artifact                  | External Adapter identity chain through installed CLI/Workbench/Fork; v1→v2 store migration; lockfile-bearing source bundle frozen install/typecheck/build/374 tests                                                              | npm publication, current-revision binding, independent Adapter/operator, cross-host or production result                                                     |
...
iterations. The first row is one unified flagship proof under one Mission
identity; the remaining rows preserve the narrower per-iteration and package
records. Every row retains its own artifact, revision, environment, and claim
boundary. The unified row does not raise the other records to its scope or turn
one same-host run into a stronger evidence level.
...
documented Runtime Profile boundary. The unified flagship is one same-host
controlled-fixture run; its provider termination and failing tool probe are
induced observation boundaries, not natural-failure or general reliability
evidence. Its standalone checker is not hosted CI or independent third-party
reproduction. The Iteration 6 planner result is a controlled-interruption
demonstration rather than a natural-failure benchmark; the Iteration 8 proof is
a controlled same-host fixture rather than production, cross-host, independent,
provider-internal, or natural-failure evidence; the Iteration 9 proof is a
controlled same-host Qoder run plus an outside-repository process, not hosted
CI, cross-host, third-party, or Profile-only causal evidence; and the Iteration
10 smoke does not establish registry publication, current-revision binding, or
independent external installation.
