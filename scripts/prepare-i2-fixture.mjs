...
const template = join(repositoryRoot, 'examples', 'i2-fixture', 'template');
const requested = process.argv[2];
const workspace =
  requested === undefined
    ? mkdtempSync(join(tmpdir(), 'missionbraid-i2-'))
...
run(
  ...
  [
    ...
    'commit',
    '-qm',
    'controlled Iteration 2 fixture',
  ],
  workspace,
);
...
if (baseline.status === 0) {
  throw new Error('Iteration 2 fixture unexpectedly passes before any Runtime Attempt');
}
...
function prepareEmptyDirectory(path) {
  if (existsSync(path)) throw new Error(`Refusing to overwrite an existing path: ${path}`);
